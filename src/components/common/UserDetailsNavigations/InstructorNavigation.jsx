...
const InstructorNavigation = (params) => {
  ...
  return (
    <div className={classes['details__actions']}>
      <Link className={classes['details__actions-btn']} to={`/feedback/list?instructor_id=${instructor_id}`}>
        <Button size='large'>Feedback</Button>
      </Link>
      <Link className={classes['details__actions-btn']} to={`/class/list?instructor_id=${instructor_id}`}>
